...
run() {
  ...
  printf '%s→%s %s\n' "$C_BLUE" "$C_RESET" "$desc"
  # Команда печатается и при обычном применении, а не только в dry. Пока о
  # каждом шаге спрашивали отдельным окном, она была видна в этом окне; теперь
  # окна нет, и без этой строки поток превратился бы в список намерений без
  # единого доказательства, что именно выполнялось. Условие — то же, что и у
  # diff в run_write: не повторять то, что человек прочитал секунду назад.
  if (( ! asked )) || ui_has_whiptail; then
    printf '%s    %s%s\n' "$C_DIM" "$rendered" "$C_RESET"
  fi
...
run_write() {
  ...
  # и остаётся в прокрутке ровно там, где случилась правка.
  #
  # Ровно один раз: в пошаговом режиме без whiptail его только что напечатал
  # сам вопрос. Окно newt, наоборот, закрылось и унесло diff с собой — да ещё
  # и показывало его обрезанным, — так что после окна печатать надо.
  if (( ! asked )) || ui_has_whiptail; then
    local diff_line
